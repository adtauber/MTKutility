...
public class ProgressCircle extends View {
    ...
    private void init(@NonNull Context context, @Nullable AttributeSet attrs) {
        ...
        if (attrs != null) {
            ...
            progressColor = a.getColor(R.styleable.ProgressCircle_progressColor, progressColor);
            progressBackgroundColor = a.getColor(R.styleable.ProgressCircle_progressBackgroundColor, progressBackgroundColor);
            progressStrokeWidth = a.getDimensionPixelSize(R.styleable.ProgressCircle_progressStrokeWidth, progressStrokeWidth);
            progressBackgroundStrokeWidth = a.getDimensionPixelSize(R.styleable.ProgressCircle_progressBackgroundStrokeWidth, progressStrokeWidth);
            textColor = a.getColor(R.styleable.ProgressCircle_textColor, progressColor);
            textSize = a.getDimensionPixelSize(R.styleable.ProgressCircle_textSize, textSize);

            shouldDrawDot = a.getBoolean(R.styleable.ProgressCircle_drawDot, true);
            dotColor = a.getColor(R.styleable.ProgressCircle_dotColor, progressColor);
            dotWidth = a.getDimensionPixelSize(R.styleable.ProgressCircle_dotWidth, progressStrokeWidth);

            startAngle = a.getInt(R.styleable.ProgressCircle_startAngle, DEFAULT_PROGRESS_START_ANGLE);
            if (startAngle < 0 || startAngle > 360) {
                startAngle = DEFAULT_PROGRESS_START_ANGLE;
            }

            isAnimationEnabled = a.getBoolean(R.styleable.ProgressCircle_enableProgressAnimation, true);
            isFillBackgroundEnabled = a.getBoolean(R.styleable.ProgressCircle_fillBackground, false);

            direction = a.getInt(R.styleable.ProgressCircle_direction, DIRECTION_COUNTERCLOCKWISE);

            int cap = a.getInt(R.styleable.ProgressCircle_progressCap, CAP_ROUND);
            progressStrokeCap = (cap == CAP_ROUND) ? Paint.Cap.ROUND : Paint.Cap.BUTT;

            String formattingPattern = a.getString(R.styleable.ProgressCircle_formattingPattern);
